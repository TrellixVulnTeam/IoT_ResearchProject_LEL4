...
head(df) %>% kable(format = "latex", caption = "Add text", 
                   booktabs = T) %>% 
  kable_styling(latex_options = c("hold_position", "striped", "scale_down"),
                bootstrap_options = c("striped", "hover", "condensed"),
                font_size = 8)
...
This data structure thus offers the following benefit;

- Simultaneous consideration of the state of multiple sub-activities
- Present in a format that is still readily human interprettable
- Present in a format that can be easily consumed by a machine

And the following challenges:

- The dataset is overall very large, potentially requiring large amounts of computational power for downstream analysis
- The dataset is potentially highly homogeneous in certain temporal regions
- A second-by-second analysis is most likely to granular for the purposes of this work
...
27/3/2003  6:43:40 am
11/4/2003  10:24:18 pm

If the sub-actitivity is occuring at that point in time, the value in the cell will be `1`. If the 
As demonstrated 



bathroom_toiletflush	bathroom_lightswitch	foyer_lightswitch	bathroom_medicinecabinet	bathroom_cabinet	bathrooom-sinkfaucet-hot	bathroom_showerfaucet	bathrooom-sinkfaucet-cold	kitchen_laundrydryer	bathroom_exhaustfan	bathroom_door
